...
1. 打开网站，点进「高中」或「大学」，再点进某个小板块（如「学科竞赛」），底部会**出现「✎ 编辑模式」按钮**（首页、封面、以及高中/大学的外层板块列表都不显示），点它。
2. 第一次会弹出输入框，粘贴你的 Token，点「解锁」（或点「取消」退出）。
3. 解锁后进入编辑模式，底部变成三个按钮：「直接退出」「保存并退出」「发布」。
...
点底部「**发布**」。网页会把新图片和内容提交到 GitHub，约几十秒～1 分钟后，所有访客刷新就能看到新内容，然后自动退出编辑模式。

## 注意事项

- **图片会被压缩**到最长边 1600px、约 200~500KB（GitHub 接口有 1MB 上限，同时加快网页加载）。存的是压缩版，不是手机原图。
- **编辑时底部的三个按钮**：`直接退出` = 放弃所有未发布的修改；`保存并退出` = 存进本地草稿、下次接着改；`发布` = 提交到 GitHub 让所有人可见。
- **未点「发布」前，改动只在你本地**（每次编辑会自动存草稿，刷新不丢），不影响访客。
